...
const dest = path.resolve(__dirname, "../contracts/libs/Canonical.sol");
...
if (process.env.NODE_ENV === "test" || chainId === "1337") {
  fs.copySync(path.join(dir, "localhost/Canonical.sol"), dest);
} else {
  fs.copySync(path.join(dir, "not-localhost/Canonical.sol"), dest);
}
